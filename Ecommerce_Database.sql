...
-- 2. Bảng lưu thông tin role  ( 'Customer', 'Staff', 'Manager', 'Admin' )
CREATE TABLE Roles (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
...
-- 3. Bảng lưu thông tin chi nhánh
...
-- 5. Bảng lưu thông tin số lượng tồn kho sản phẩm tại chi nhánh
CREATE TABLE Stocks (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
...
-- 6. Bảng lưu thông tin danh mục sản phẩm
...
-- 7. Bảng lưu thông tin nhà sản xuất
...
-- 10. Bảng quan hệ giữa sản phẩm và giá trị thuộc tính
CREATE TABLE ProductAttributes ( -- nhiều nhiều
...
-- 11. Bảng lưu thông tin địa chỉ nhận hàng
...
-- 12. Bảng lưu thông tin trạng thái order đơn hàng
...
-- 13. Bảng lưu thông tin đơn hàng
CREATE TABLE Orders (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    userId BIGINT NOT NULL,             -- Khóa ngoại tới bảng users
...
-- 15. Bảng lưu phương thức thanh toán
...
-- 16. Bảng lưu thông tin thanh toán
...
-- 17. Bảng lưu lịch sử thanh toán one-to-one
...
-- 18. Bảng lưu thông tin chat hỗ trợ
...
-- 19. Bảng lưu thông tin doanh thu
...
-- 20. Bảng lưu thông tin về việc đánh giá và rating sản phẩm, nếu là Delivered thì cho phép đánh giá
...
-- 21. Bảng lưu trữ lượt xem đơn hàng
...
-- 22. Bảng lưu trữ việc khuyến mãi của sản phẩm
...
-- 24. Bảng lưu trữ mã giảm giá đối với người dùng
CREATE TABLE Coupons (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    code VARCHAR(50) UNIQUE NOT NULL,            -- Mã giảm giá (VD: SALE2025)
    description VARCHAR(255),                   -- Mô tả khuyến mãi
    discountPercentage DECIMAL(5, 2) CHECK (discountPercentage >= 0 AND discountPercentage <= 100),           -- Giảm theo phần trăm (nếu có)
    discountAmount DECIMAL(10, 2),              -- Giảm theo số tiền cụ thể (nếu có)
    maxUses INT NOT NULL DEFAULT 1,             -- Số lần sử dụng tối đa
    usedCount INT DEFAULT 0 CHECK (usedCount <= maxUses),                    -- Số lần đã sử dụng
...
CREATE TABLE CouponUsers (
    id BIGINT PRIMARY KEY AUTO_INCREMENT,
    couponId BIGINT NOT NULL,                     -- Khóa ngoại tới bảng Coupons
    userId BIGINT NOT NULL,                       -- Khóa ngoại tới bảng Users
